...
//!
//! Taken from pwoolcoc/common-path.
//! (downloaded from crates.io, as this does not have an official repo,
//! and modified so that it supports iterators over &PathBuf, and not only &Path).
//!
//! # Example
//!
//! ```rust
...
/// # }
/// ```
pub fn common_path_all<T>(paths: impl IntoIterator<Item = T>) -> Option<PathBuf>
where T: AsRef<Path> {
    let mut path_iter = paths.into_iter();
    let mut result = path_iter.next()?.as_ref().to_path_buf();
    for path in path_iter {
        if let Some(r) = common_path(&result, path.as_ref()) {
            result = r;
        } else {
            return None;
        }
